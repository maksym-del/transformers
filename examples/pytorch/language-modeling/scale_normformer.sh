...
python -m torch.distributed.launch --nproc_per_node=$NUM_GPUS --master_port=9901 \
    run_mlm.py \
        ...
        --pad_to_max_length \
        --learning_rate $LR \
        --config_overrides="scale_post=False,scale_pre=True,scale_fc=True,scale_attn=True,scale_heads=True,scale_resids=True" \
        --report_to all \
        --run_name $RUN_NAME \
